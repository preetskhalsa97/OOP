...
//		}
		...
		final LeftThread leftThread 			= 	new LeftThread(frame,circleIcon);
		final RightThread rightThread 			= 	new RightThread(frame,circleIcon);
		final ScaleUpThread scaleUpThread 		= 	new ScaleUpThread(frame,circleIcon); 
		final ScaleDownThread scaleDownThread 	= 	new ScaleDownThread(frame,circleIcon);
		...
		rightButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				if (!rightThreadStarted){
					...
					leftThreadRunning 		= false;	//leftThread is not running initially
					scaleUpThreadRunning 		= false;	//scaleUpThread is not running initially
					scaleDownThreadRunning 	= false;
		...
		scaleDownButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				if (!scaleDownThreadStarted){
					scaleDownThreadStarted=true;
					
					rightThreadRunning= false; 
					leftThreadRunning 		= false;	
					scaleDownThreadRunning 	= true;
					scaleDownThreadRunning 	= true;
		...
		scaleUpButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				if (!scaleUpThreadStarted){
					scaleUpThreadStarted=true; 
					
					rightThreadRunning= false; 
					leftThreadRunning 	= false;	
					scaleDownThreadRunning 	= false;
					scaleUpThreadRunning = true;
					scaleUpThread.start();		
				}
				else{
