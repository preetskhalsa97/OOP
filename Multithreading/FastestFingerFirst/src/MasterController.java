				...
				while (true){
					synchronized(this){
						...
						for (int i=0;i<10;i++){
							...
							if (playerList.get(i).ans.getTimetaken()>10000){
								playerList.get(i).getButton().setForeground(Color.CYAN);
							}							
							else if (playerList.get(i).ans.getAnswerSequence()==currentQuestion.getCorrectSequence()){
						...
						ArrayList <Player> correctAns = new ArrayList <Player>(); 
						
						
						for (int i=0;i<10;i++){
							if (playerList.get(i).ans.getAnswerSequence()==currentQuestion.getCorrectSequence()){
								correctAns.add(playerList.get(i));
							}
						}
						
						if (correctAns==null){
							winnerPlayer.setText("no winner!");
						}
						
						else{
							ArrayList <Long> timeTaken = new ArrayList<Long>(); 
							for (Player i  : correctAns){
								timeTaken.add(i.ans.getTimetaken()); 
							}
							int min=0;
							for (int i=0;i<timeTaken.size();i++){
								if (timeTaken.get(i)<timeTaken.get(min)){
									min=i; 
								}								
							}
							Player win = playerList.get(min);  
							winnerPlayer.setText(win.toString());
						}					
